...
At a high level, Literature Agent is a full-stack AI research assistant focused on AI/ML topics.
...
This makes the app useful as a demonstration of agent orchestration, retrieval grounding, and AI-assisted research workflow design, rather than as a production-grade scientific discovery platform.
...
- a generic AI/ML fallback search if too few papers are found.
...
### 4. API key rotation and fallback logic
Gemini key management is built to support:
...
DeepSeek is included as a fallback JSON-generation path for some model tasks.

### 5. Real backend experiment execution
...
### 6. Report generation with self-checking
...
The `TopicRefinementAgent` transforms a broad topic into a narrower AI/ML-oriented research question that is more appropriate for the rest of the pipeline.
...
- falls back to broader generic AI/ML queries if needed.
...
### AI services
- **Gemini** is the primary model provider.
- **DeepSeek** is available as a fallback path.
...
### Why this matters
Many “AI research assistant” demos only generate narrative text. This project at least executes an actual backend ML routine and feeds those outputs into later analysis.
...
Literature Agent is a thoughtful prototype for grounded AI-assisted research workflow generation. Its main strengths are not that it “solves research,” but that it combines:
